    ...
    get grpc() {
        ...
        if (!this[GRPC]) {
            ...
            for (let key in serviceConfig) {
                ...
                let appReference = this
                temp[key] = proxyHandler(appReference, groupName, groupVersion, instance)
            }
            this[GRPC] = temp
        }
        return this[GRPC];
...
function proxyHandler(appReference, groupName, groupVersion, pbInstance) {
    let cache = {}
    return new Proxy({}, {
        get: function (target, key, receiver) {
            return function (params, metaData = {}) {
                const services = appReference.services
                ...
                if (cache[host]) {
                    ...
                    const pemPath = appReference.config.grpc.pem
                    ...
                    client = cache[host] = new pbInstance(host, combined_creds, appReference.config.grpc.options)
                }
                let retry = 0
                const maxRetry = appReference.config.grpc.retry || 5 // 最大重试次数， 默认5次
